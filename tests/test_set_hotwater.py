...
async def test_set_hot_water(aresponses: ResponsesMockServer, monkeypatch: Any) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        request_mock.assert_called_with(base_path="/JS", data={
            "Parameter": "1600",
            "EnumValue": "3",
            "Type": "1"
        })
...
async def test_prepare_hot_water_state() -> None:
    ...
    state = bsblan._prepare_hot_water_state(operating_mode="3", nominal_setpoint=None, reduced_setpoint=None)
    assert state == {
        "Parameter": "1600",
        "EnumValue": "3",
        "Type": "1"
    ...
    state = bsblan._prepare_hot_water_state(operating_mode=None, nominal_setpoint=60.0, reduced_setpoint=None)
    assert state == {
        "Parameter": "1610",
        "Value": "60.0",
        "Type": "1"
    ...
    state = bsblan._prepare_hot_water_state(operating_mode=None, nominal_setpoint=None, reduced_setpoint=40.0)
    assert state == {
        "Parameter": "1612",
        "Value": "40.0",
        "Type": "1"
    ...
    with pytest.raises(BSBLANError, match=NO_STATE_ERROR_MSG):
        bsblan._prepare_hot_water_state(operating_mode=None, nominal_setpoint=None, reduced_setpoint=None)

@pytest.mark.asyncio
async def test_set_hot_water_state(aresponses: ResponsesMockServer, monkeypatch: Any) -> None:
